-- Backfill baseline structured effect_json for all skills that are still null.
-- Keep previously curated effect_json untouched.
...
FROM derived d
WHERE s.id = d.id
  AND s.effect_json IS NULL;

COMMIT;

-- Verification
SELECT COUNT(*) AS total,
       COUNT(effect_json) AS structured,
       COUNT(*) - COUNT(effect_json) AS unstructured
FROM skills;
